...
- `bundles/dsh-plugins`（`dsh-plugins`）：只负责显式组合 `authorization`、`codex-login` 和 `system-notification`，不承载业务实现。
- `dsh/`（若存在）：DSH 全局目录的源文件，部署到用户全局的 `~/.dsh/`；修改时要区分全局规则与本仓库插件代码。
...
- `bundles/dsh-plugins/cordis.patch.yml` 必须显式插入 `authorization`、`codex-login` 和 `system-notification`；不要依赖 DSH 递归激活 Bundle，也不要把插件实现复制到聚合目录。
- `/init` 只为旧配置兼容保留。其 Host 通过 `commands` 注册无参数的 `/init`，只允许 Agent 修改仓库根目录的 `AGENTS.md`，不创建 Goal，也不使用 `goal-round-driver`。
- `packages/codex-login/src/host.mjs` 依赖 `authorization` 和 `webServer`，提供 `/api/codex-login/start`、`poll`、`answer`、`cancel` 四个接口；修改授权状态机、提示投影或 effect 清理时，要同时考虑重复请求、取消和卸载。
...
2. 修改聚合关系时，用已有的 Web Profile 执行 `dsh --profile web --dump-config`，确认没有重复或缺失的 layer。
