...
![Session 费用明细](assets/session-cost-details.jpg)
...
- **与 Codex 用量插件并装**：两者都位于 `conversation.composer.dock`；本插件替换内置的 `stats`（`order: 0`）条目，Codex 用量插件使用独立的 `codex-usage`（`order: 1`）条目，按当前 DSH Slot 契约可同时安装。
