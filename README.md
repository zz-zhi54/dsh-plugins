...
{
  "name": "dsh-plugins",
  "version": "0.1.6-alpha.2.1",
  "tags": ["deepseek", "deepseek-harness", "dsh", "plugins"],
  "description": "非官方 DeepSeek Harness 插件集合，可从 GitHub 按需安装",
  "icon": "https://raw.githubusercontent.com/zz-zhi54/dsh-plugins/v0.1.6-alpha.2.1/packages/system-notification/assets/dsh.ico",
  "compatible_versions": ["v0.1.6-alpha.2"],
...
当前兼容 DSH `v0.1.6-alpha.2`；未发现 DSH 破坏性契约变化，`session-cost` 已改用新的投影 seam，避免新增依赖已弃用的 Session 快照读取。
...
| --- | --- | --- | --- |
| `packages/system-notification` | `dsh-system-notification-plugin` | `0.1.6-alpha.2.1` | macOS / Windows 系统通知 |
| `packages/codex-login` | `dsh-codex-login-plugin` | `0.1.6-alpha.2.1` | 首次 ChatGPT / Codex 登录 |
| `packages/codex-usage` | `dsh-codex-usage-plugin` | `0.1.6-alpha.2.1` | Codex 额度显示 |
| `packages/session-cost` | `dsh-session-cost-plugin` | `0.1.6-alpha.2.1` | Session 费用显示 |
| `packages/project-mcp` | `dsh-project-mcp-plugin` | `0.1.6-alpha.2.1` | 项目级 MCP 隔离 |
...
固定 release 时，在路径前加入版本标签。以版本 `v0.1.6-alpha.2.1` 为例：

```sh
# 系统通知
dsh plugin --profile web add 'github:zz-zhi54/dsh-plugins#v0.1.6-alpha.2.1&path:packages/system-notification'

# Codex 登录
dsh plugin --profile web add 'github:zz-zhi54/dsh-plugins#v0.1.6-alpha.2.1&path:packages/codex-login'

# Codex 额度
dsh plugin --profile web add 'github:zz-zhi54/dsh-plugins#v0.1.6-alpha.2.1&path:packages/codex-usage'

# Session 费用
dsh plugin --profile web add 'github:zz-zhi54/dsh-plugins#v0.1.6-alpha.2.1&path:packages/session-cost'

# 项目级 MCP
dsh plugin --profile web add 'github:zz-zhi54/dsh-plugins#v0.1.6-alpha.2.1&path:packages/project-mcp'
```
